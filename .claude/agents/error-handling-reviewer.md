...
description: Kotlin 에러처리 패턴(runCatching/Result/코루틴)을 검토하고 구체적 수정안을 제시한다. 코드가 runCatching·Result·코루틴 cancellation 경계를 다룰 때, 에러 전파/예외 처리 품질을 검증할 때 사용한다. 읽기 전용 — 코드를 직접 수정하지 않고 before/after 스니펫으로 제안만 한다.
