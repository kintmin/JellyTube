...
class StepSensorProcessorTest {
    ...
    fun setUp() {
        ...
        processor = StepSensorProcessor(
            ...
            onBackupSensor = { sensor, saveMillis ->
                if (processor.truncateToUnitZonedDateTime(saveMillis, utc).toInstant().toEpochMilli() == saveMillis) {
    ...
    private fun utcMillis(hour: Int, minute: Int, second: Int = 0): Long =
        ZonedDateTime.of(2024, 1, 15, hour, minute, second, 0, utc).toInstant().toEpochMilli()

    // ─── truncateToUnitZonedDateTime ────────────────────────────────────────

    @Test
    fun `00시 00분 00초는 00시 00분 버킷에 속한다`() {
        val result = processor.truncateToUnitZonedDateTime(utcMillis(0, 0, 0), utc)
    ...
    fun `00시 29분 59초는 경계 직전으로 00시 00분 버킷에 속한다`() {
        val result = processor.truncateToUnitZonedDateTime(utcMillis(0, 29, 59), utc)
        assertEquals(0, result.hour)
        assertEquals(0, result.minute)
    }
    ...
    fun `00시 30분 00초는 00시 30분 버킷 정각이다`() {
        val result = processor.truncateToUnitZonedDateTime(utcMillis(0, 30, 0), utc)
        assertEquals(0, result.hour)
    ...
    fun `23시 59분 59초는 23시 30분 버킷에 속한다`() {
        val result = processor.truncateToUnitZonedDateTime(utcMillis(23, 59, 59), utc)
    ...
    fun `truncate 결과의 초와 나노초는 항상 0이다`() {
        val result = processor.truncateToUnitZonedDateTime(utcMillis(9, 47, 33), utc)
        assertEquals(0, result.second)
    ...
    fun `lastBackupUnitMillis가 null일 때 첫 버킷 변경 시 saveMillis는 currentUnitMillis다`() {
        ...
        val expectedSaveMillis = processor.truncateToUnitZonedDateTime(eventMillis, utc).toInstant().toEpochMilli()
    ...
    fun `saveMillis는 lastBackupUnitMillis에 backupUnit을 더한 값이다`() {
        ...
        val expected = processor.truncateToUnitZonedDateTime(utcMillis(9, 0), utc).toInstant().toEpochMilli() + backupUnit
